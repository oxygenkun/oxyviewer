...
| YuNet 2023mar, SFace 2021dec | URLs, sizes, SHA-256, and licenses in `3rdpart/face-models/source.json` | ONNX model files downloaded into `target/native/face-models` and executed by the pure-Rust `tract` runtime inside `oxy-faces`; no native library is linked |
...
Face model files are downloaded inputs rather than compiled artifacts, so they
are pinned by URL plus SHA-256 and by the Git-LFS object id recorded upstream.
They are not part of `native:prepare`, because the face analyzer is optional and
must not gate a media build.
