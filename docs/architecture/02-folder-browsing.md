...
这阻止前端仅通过构造路径就越过用户打开的根目录。session 目前只存于内存，应用重启后失效。
...
`list_directories` 同样只列当前目录的直接子目录。为了显示是否有展开标记，它会对每个子目录
再做一次浅层 `read_dir`，只判断是否存在目录子项。结果按不区分大小写的名称排序并缓存。

这个 `hasChildren` 探测在子目录非常多时可能成为可测成本；修改时应保持“浅层存在性检查”，
不能演变为递归构建整棵目录树。
...
| 目录树 | 浅层 children 探测 | 大目录下按需或批量优化 |
