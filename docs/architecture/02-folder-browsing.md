...
- 打开不递归；
- 首屏不读完整元数据，不解码图片；
- 当前目录只建立轻量摘要；已完成的资料库索引可直接提供磁盘缓存分页；
- 摘要快照在内存中复用；
- IPC 每次最多返回一页；
- React 只把可见行附近的节点挂到 DOM。

这四层经常被混淆：后端分页减少 IPC 数据量，虚拟列表减少 DOM 数量。首次索引尚未完成时，
后端仍会扫描当前目录的全部直接文件形成内存快照；索引可用后，普通列表与名称搜索直接通过
SQLite 排序和分页。打开 command 本身始终不等待递归索引。
...
目录扫描，使用 `oxy-fs::scan_index_directory` 按单层目录取得轻量文件摘要与直接子目录：

1. 每层扫描后用一个短 SQLite transaction upsert `indexed_assets` 和 `indexed_directories`；
2. transaction 结束后立即释放 connection，搜索不会等待整个根目录扫完；
3. 重建期间保留上一次完整索引；最后删除旧 generation，并一次更新 FTS；
4. 首次索引完成前回退第 4 节的内存快照；完成事件使 React Query 自动重新获取；
5. 移除根目录会同时删除其索引，扫描器也会在目录边界检查 root 是否仍然注册。
...
无索引时，`page_assets` 对快照执行过滤和排序。索引可用时，相同 query 由 SQLite 执行 kind、
sort、direction、LIMIT/OFFSET；非空名称搜索改为根目录范围的 FTS5 前缀查询，同时匹配文件名和
...
目录搜索不在索引缺失时退化为同步递归磁盘扫描。首次索引尚未完成的根显示建立索引状态，
完成事件使对应 React Query 自动重取；其他已完成根的搜索结果仍可先显示。
