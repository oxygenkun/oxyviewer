...
连拍分组同样不在打开路径上：网格首屏渲染后才调用 `scan_burst_groups`，由
`oxy_metadata::BurstScanner` 增量读取尚未见过的文件的 maker note（每次调用只解析最新
一页），分组本身是 `oxy_domain::group_bursts` 的纯计算。分页可能在前一轮解析完成前发布下一页，
因此扫描器用独立 admission gate 串行化增量扫描，防止并发请求重复解析相同前缀。扫描只从原生
标签解析器提取拍摄时间、释放模式和连拍序号，不进入完整 MetadataDocument、对焦、HEIF XMP 或
色度读取路径。前端先提交覆盖首屏的 24 项，再以 64 项批次扩展已扫描前缀；每批结果立即更新网格，
因此首屏连拍不等待整页路径全部识别完毕，后续页也复用同一扫描缓存。

完整 metadata 与连拍不能合并成同一个强制任务：前者还要构造 raw tags、对焦信息，并为 HEIF
读取 XMP/色度；后者要覆盖整个已加载列表，但只需要三个拍摄字段。让连拍统一走完整文档会放大
打开后的 I/O 和内存。可共享的边界是更低层的原生标签解析：后续可以加入按 source identity
校验、容量受限且支持同路径 single-flight 的 parsed-tags 缓存，使同时发生的完整 metadata 与连拍
只解析一次；缓存命中后两边仍各自构造轻量连拍帧或完整文档。未具备身份校验和容量上限前，不应
直接长期缓存整个文件夹的 `Vec<Tag>`。
...
共享不可变快照，`RwLock` 保护目录到快照的映射。目录树与资料库索引现在共用
`directory_cache`：按 canonical path 设置的单飞 gate 保证两条后台路径并发请求同一层时只执行
一次子目录枚举。索引仍在每个条目之间检查前台让行门控；共享不能把大目录扫描变成阻塞首屏的
临界区。
...
执行刷新时，后端删除当前资产快照，并清除当前根下面的共享目录缓存，前端取消相关 query、调用
刷新，再使 query 失效重取。
...
这两个阶段不会再无条件重复其他消费者刚完成的工作。目录阶段通过 `FsCatalog` 与目录树共享
每层的单飞枚举结果；资产阶段可复用本次 index generation 启动后由前台浏览发布的已验证目录
快照。较老的进程内快照不会被索引接受，避免一次新的重建悄悄沿用历史目录内容。
