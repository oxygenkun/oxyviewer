...
状态：已随 0.1.3 发布（2026-09-17）；Phase 1 与 Phase 2 已落地（见 §10）。
真机手工矩阵与 release 构建性能对照仍未完成，因此本文留在 `docs/tasks/`。
...
- 2026-09-17：随 0.1.3 发布。原生拖入与侧边栏 `+` 两个入口及底栏反馈均进入发布版本；
  真机手工矩阵与 release 构建性能对照仍未执行，见 §7。
