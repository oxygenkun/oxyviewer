//! HEIF request execution, runtime probes, fallback, and diagnostics.
//! Pure candidate ordering and request policy live in `planner`.
...
use planner::{
    AttemptDiagnostic, AttemptOutcome, BackendProbe, BackendProbes, Platform, ProbeState,
    plan_backends,
};
pub(crate) use planner::{HeifBackend, HeifBackendPlan, HeifOperation};
use std::path::Path;
...
pub(crate) fn preview(
    ...
    priority: crate::decode_control::DecodePriority,
    allow_interim: bool,
    ...
    if level == RenderLevel::Full {
        match artifact::full(path, cache_dir, cancellation) {
            Ok(result) => return Ok(result),
            Err(error) if !heif_full_error_allows_fallback(&error) => return Err(error),
            Err(error) => eprintln!(
    ...
    let plan = planner::HeifPreviewPlan::for_level(level);
    artifact::preview(
        path,
        cache_dir,
        plan.max_size,
...
pub(crate) fn probe_backend_plan(path: &Path, operation: HeifOperation) -> HeifBackendPlan {
    plan_backends(
        current_platform(),
        operation,
...
mod tests {
    ...
    #[test]
    fn semantic_levels_preserve_embedded_preview_and_native_full_fallback() {
        use oxy_domain::RenderLevel;
        for (level, max_size, embedded, native_detail) in [
            (RenderLevel::Thumbnail, 512, true, false),
            (RenderLevel::Preview, 4_096, true, false),
            (RenderLevel::Full, 8_192, false, true),
        ] {
            let plan = planner::HeifPreviewPlan::for_level(level);
            assert_eq!(
                (
                ...
                (max_size, embedded, native_detail)
            );
        }
    }
    ...
    fn selector_preserves_preview_and_session_backend_order() {
        ...
        let windows_session = plan_backends(
            Platform::Windows,
            HeifOperation::Session,
            probes(supported(), supported()),
        );
        ...
        let linux_session = plan_backends(
            Platform::Linux,
            HeifOperation::Session,
            probes(supported(), supported()),
        );
        ...
        let mac_artifact = plan_backends(
            Platform::Macos,
